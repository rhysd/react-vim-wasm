...
export class VimWorker {
    ...
    awakeWorkerThread(event) {
        if (this.debug) {
            const status = Atomics.load(this.sharedBuffer, 0);
            if (status !== 0) {
                console.error('INVARIANT ERROR! Status byte must be zero cleared:', status);
            }
        }
...
export class ScreenCanvas {
    constructor(worker, canvas, input) {
        ...
        this.canvas.width = rect.width * res;
        this.canvas.height = rect.height * res;
        this.canvas.addEventListener('click', this.onClick.bind(this), { capture: true, passive: true });
    ...
    drawText(text, ch, lh, cw, x, y, bold, underline, undercurl, strike) {
        ...
        for (let i = 0; i < text.length; ++i) {
            ...
            console.log('fillText:', c, Math.floor(x + cw * i), yi);
        ...
        if (underline) {
            ...
            console.log('moveTo:', Math.floor(x), underlineY);
            console.log('lineTo:', Math.floor(x + cw * text.length), underlineY);
            console.logmoveTo(Math.floor(x), underlineY);
            ...
            console.log('moveTo:', Math.floor(x), undercurlY);
            console.log('lineTo:', Math.floor(x + cw * text.length), undercurlY);
...
export class VimWasm {
    ...
    sendKeydown(key, keyCode, modifiers) {
        const { ctrl: ctrl = false, shift: shift = false, alt: alt = false, meta: meta = false } = modifiers || {};
        if (key.length > 1) {
            if (
                key === 'Unidentified' ||
                (ctrl && key === 'Control') ||
